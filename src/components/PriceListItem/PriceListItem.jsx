...
const PriceListItem = ({ id, title, article, price, group, addition }) => {
    ...
    const addToOrderCalculator = () =>
        dispatch(addProductItem({ id, title, article, price, group }));
    const addServiceToOrderCalculator = () =>
        dispatch(addService({ id, title, addition, price, group }));
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                onClick={
                    isSpare ? addToOrderCalculator : addServiceToOrderCalculator
                }
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                ...
                <div className="products__inner__item__price">
                    ...
                    {isSpare ? (
                        <div className="price__count">
                            {parseInt(price).toLocaleString("ru")} ₽
                        </div>
                    ) : (
